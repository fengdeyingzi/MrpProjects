...
static int nextrune(struct cstate *g)
{
	...
	if (g->yychar == '\\') {
		...
		if (mrc_strchr(ESCAPES, g->yychar))
			return 1;
...
static int lexclass(struct cstate *g)
{
	...
	for (;;) {
		...
		} else if (quoted && mrc_strchr("DSWdsw", g->yychar)) {
			if (havesave) {
...
{
	Rune *p;
	if (!node) { mrc_printf("Empty"); return; }
	switch (node->type) {
	case P_CAT: mrc_printf("Cat("); dumpnode(node->x); mrc_printf(", "); dumpnode(node->y); mrc_printf(")"); break;
	case P_ALT: mrc_printf("Alt("); dumpnode(node->x); mrc_printf(", "); dumpnode(node->y); mrc_printf(")"); break;
	case P_REP:
		mrc_printf(node->ng ? "NgRep(%d,%d," : "Rep(%d,%d,", node->m, node->n);
		dumpnode(node->x);
		mrc_printf(")");
		break;
	case P_BOL: mrc_printf("Bol"); break;
	case P_EOL: mrc_printf("Eol"); break;
	case P_WORD: mrc_printf("Word"); break;
	case P_NWORD: mrc_printf("NotWord"); break;
	case P_PAR: mrc_printf("Par(%d,", node->n); dumpnode(node->x); mrc_printf(")"); break;
	case P_PLA: mrc_printf("PLA("); dumpnode(node->x); mrc_printf(")"); break;
	case P_NLA: mrc_printf("NLA("); dumpnode(node->x); mrc_printf(")"); break;
	case P_ANY: mrc_printf("Any"); break;
	case P_CHAR: mrc_printf("Char(%c)", node->c); break;
	case P_CCLASS:
		mrc_printf("Class(");
		for (p = node->cc->spans; p < node->cc->end; p += 2) mrc_printf("%02X-%02X,", p[0], p[1]);
		mrc_printf(")");
		break;
	case P_NCCLASS:
		mrc_printf("NotClass(");
		...
		mrc_printf(")");
		break;
	case P_REF: mrc_printf("Ref(%d)", node->n); break;
	}
}
		...
		if (argc > 2) {
			...
			if (!regexec(p, s, &m, 0)) {
				for (i = 0; i < m.nsub; ++i) {
					...
					if (n > 0)
						mrc_printf("match %d: s=%d e=%d n=%d '%.*s'\n", i, (int)(m.sub[i].sp - s), (int)(m.sub[i].ep - s), n, n, m.sub[i].sp);
					else
						mrc_printf("match %d: n=0 ''\n", i);
				}
			} else {
				mrc_printf("no match\n");
			}
