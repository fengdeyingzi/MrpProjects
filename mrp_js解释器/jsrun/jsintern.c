...
void js_putc(js_State *J, js_Buffer **sbp, int c)
{
	...
	if (!sb) {
		...
		*sbp = sb;
		...
		sb = js_realloc(J, sb, (sb->m *= 2) + soffsetof(js_Buffer, s)); //xldebug
		*sbp = sb;
	}
	sb->s[sb->n++] = c;
}
...
static js_StringNode jsS_sentinel;
...
//xldebug
void jsS_init(){
	//  = { &jsS_sentinel, &jsS_sentinel, 0, ""}
	jsS_sentinel.left = &jsS_sentinel;
	jsS_sentinel.right = &jsS_sentinel;
	jsS_sentinel.level = 0;
	jsS_sentinel.string[0] = '\0';
}