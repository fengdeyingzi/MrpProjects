...
BITMAP_565* bmp_read(void *buf, int len){
 ...
 if(bufc[0]=='B' && bufc[1]=='M'){
  ...
  //  debug_printf("16位图\n");
   ...
   bmp->width = w;
   bmp->height = h;
   bmp->bitmap = (uint16*)malloc(w*h*2);
   //复制位图数据
   
   for( iy=0;iy<bmp->height;iy++){
    memcpy(bmp->bitmap+iy*bmp->width, bufc+ptr+(bmp->height-1-iy)*bmp->width*2, w*2);
    //printf("复制位图数据%d %d\n",iy, bmp->height-1-iy);
   }
  ...
  //  debug_printf("当前位图是24位");
   ...
   //debug_printf("申请内存\n");
   bmp->bitmap = (uint16*)malloc(w*h*3);
   //32转16位
    buf16 = (unsigned short*)malloc(bmp->width*bmp->height*2);
    buf24 = (bufc+ptr);
   for(i=0;i<h;i++){
	   for(iw=0;iw<w;iw++){
   ...
   //复制位图数据
    for(iy=0;iy<bmp->height;iy++){
     ...
     memcpy (bmp->bitmap + iy*bmp->width, buf16+(bmp->height-1-iy)*bmp->width,w*2);
   }
   free(buf16);
  }
 }
...
void bmp_settranscolor(BITMAP_565* bmp, int color){
 bmp->transcolor = color; //((color>>8)&0xf800) | ((color>>5)&0x07e0) | ((color>>3)&0x1f);
 bmp->mode = BM_TRANSPARENT;
}
//显示bmp
void bmp_draw(BITMAP_565* bmp, int x,int y){
 mrc_printf("bmpdraw %d \n",bmp->width);
 mrc_bitmapShowFlip(bmp->bitmap, (int16)x,(int16)y, (uint16)bmp->width, (uint16)bmp->width,(uint16)bmp->height, (uint16)bmp->mode, 0,0,(uint16)bmp->transcolor);
}
//区域显示bmp
void bmp_drawflip(BITMAP_565* bmp, int x,int y,int w,int h,int tx,int ty){
 mrc_printf("bmpshowflip width=%d w=%d\n",bmp->width,w);
 mrc_bitmapShowFlip(bmp->bitmap, (int16)x, (int16)y, (uint16)bmp->width, (uint16)w,(uint16)h, (uint16)bmp->mode, (int16)tx,(int16)ty,(uint16)bmp->transcolor);
}
//bmp释放
