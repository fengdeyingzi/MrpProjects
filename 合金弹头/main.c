...
void drawPlayer(void) {
    ...
    /* 身体 */
    gl_drawRect(x + 3, y + 8, 9, 10, 0xFFC03928);  /* 马甲 */

    /* 腿 */
    gl_drawRect(x + 4, y + 18, 3, 7, 0xFF5D4037);
    gl_drawRect(x + 9, y + 18, 3, 7, 0xFF5D4037);

    /* 头 */
    gl_drawRect(x + 4, y, 7, 8, 0xFFF1C40F);  /* 脸 */
    gl_drawRect(x + 4, y + 1, 7, 2, 0xFFFFFFFF);  /* 头带 */

    /* 眼睛 */
    if (player.base.facingRight) {
        gl_drawRect(x + 9, y + 4, 2, 1, 0xFF000000);
    } else {
        gl_drawRect(x + 5, y + 4, 2, 1, 0xFF000000);
    }

    /* 枪 */
    if (player.base.facingRight) {
        gl_drawRect(x + 10, y + 10, 10, 3, 0xFF555555);
    } else {
        gl_drawRect(x - 5, y + 10, 10, 3, 0xFF555555);
    }
}
...
void drawEnemies(void) {
    ...
    for (i = 0; i < MAX_ENEMIES; i++) {
        ...
        if (enemies[i].base.type == ENTITY_ENEMY_SOLDIER) {
            /* 普通士兵 */
            gl_drawRect(x, y + 5, 15, 20, color);  /* 身体 */
            gl_drawRect(x, y, 15, 6, 0xFF95A5A6);  /* 头盔 */

            /* 枪 */
            if (enemies[i].base.facingRight) {
                gl_drawRect(x + 10, y + 13, 10, 2, 0xFF000000);
            } else {
                gl_drawRect(x - 5, y + 13, 10, 2, 0xFF000000);
            }

        } else if (enemies[i].base.type == ENTITY_ENEMY_ELITE) {
            /* 精英士兵 - 稍大 */
            gl_drawRect(x, y + 6, 20, 19, color);  /* 身体 */
            gl_drawRect(x + 2, y, 16, 7, 0xFF7F8C8D);  /* 头盔 */

            /* 枪 */
            if (enemies[i].base.facingRight) {
                gl_drawRect(x + 15, y + 12, 12, 3, 0xFF000000);
            } else {
                gl_drawRect(x - 7, y + 12, 12, 3, 0xFF000000);
            }

        } else if (enemies[i].base.type == ENTITY_ENEMY_TANK) {
            /* Boss坦克 */
            /* 履带 */
            gl_drawRect(x + 5, y + 32, 50, 8, 0xFF5D6D7E);

            /* 车身 */
            gl_drawRect(x, y + 15, 60, 17, 0xFF2874A6);

            /* 炮塔 */
            gl_drawRect(x + 15, y, 30, 15, 0xFF1B4F72);

            /* 炮管 */
            gl_drawRect(x - 10, y + 5, 25, 5, 0xFF000000);

            /* Boss血条 */
            gl_drawRect(x, y - 10, 60, 5, 0xFF333333);
            gl_drawRect(x, y - 10, (60 * enemies[i].base.hp) / enemies[i].base.maxHp, 5, 0xFFFF0000);
        }
    }
}
...
void drawParticles(void) {
    ...
    for (i = 0; i < MAX_PARTICLES; i++) {
        ...
        gl_drawRect(x, y, 2, 2, particles[i].color);
    }
}
...
void drawPlatforms(void) {
    ...
    for (i = 0; i < currentLevelData.platformCount; i++) {
        ...
        if (p->type == PLATFORM_SOLID) {
            gl_drawRect(x, y, w, h, 0xFF795548);  /* 泥土 */
            gl_drawRect(x, y, w, 5, 0xFF27AE60);  /* 草皮 */
        } else {
            gl_drawRect(x, y, w, h, 0xFF555555);  /* 钢板 */
        }
    }
}
...
void drawUI(void) {
    ...
    uc3_drawText(buf, 5, 5, 255, 255, 255, 0);

    /* 关卡 */
    mrc_sprintf(buf, "LEVEL %d", gameState.currentLevel);
    uc3_drawText(buf, 240, 5, 255, 255, 255, 0);

    /* HP条 */
    gl_drawRect(100, 8, 102, 12, 0xFF555555);
    hpWidth = (player.base.hp * 100) / player.base.maxHp;
    gl_drawRect(101, 9, hpWidth, 10, 0xFFFFCC00);

    /* Boss警告 */
    if (gameState.bossActive) {
        uc3_drawText("WARNING", 120, 30, 255, 0, 0, 0);
    }
}
...
void drawGameOver(void) {
    ...
    if (gameState.currentLevel > gameState.totalLevels) {
        /* 胜利 */
        uc3_drawText("任务完成!", 100, 80, 255, 255, 0, 0);
        mrc_sprintf(buf, "最终分数: %d", gameState.score);
        uc3_drawText(buf, 80, 110, 255, 255, 255, 0);
    } else {
        /* 失败 */
        uc3_drawText("任务失败", 100, 90, 255, 0, 0, 0);
        mrc_sprintf(buf, "分数: %d", gameState.score);
        uc3_drawText(buf, 100, 120, 255, 255, 255, 0);
    }

    uc3_drawText("按确认重新开始", 80, 160, 255, 255, 255, 0);
...
void gameEvent(int32 code, int32 param0, int32 param1) {
    if (code == KY_DOWN) {
        /* 按键按下 */
        if (param0 == _LEFT || param0 == _4) keyLeft = 1;
        if (param0 == _RIGHT || param0 == _6) keyRight = 1;
        if (param0 == _UP || param0 == _2) keyUp = 1;
        if (param0 == _DOWN || param0 == _8) keyDown = 1;
        ...
        /* 按键松开 */
        if (param0 == _LEFT || param0 == _4) keyLeft = 0;
        if (param0 == _RIGHT || param0 == _6) keyRight = 0;
        if (param0 == _UP || param0 == _2) keyUp = 0;
        if (param0 == _DOWN || param0 == _8) keyDown = 0;
...
/* 初始化 */
int32 mrc_init(void) {
    mrc_printf("[INIT] Metal Slug Starting...\n");
    mpc_init();
    /* 初始化字体 */
    uc3_init();
