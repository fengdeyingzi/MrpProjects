//#include "c/菜单_mrpoid/xl.h"
//#include "c/菜单_mrpoid/list.h"
//#include <ui_list.h>
#include "xl.h"
enum
{
...
extern int ui_textevent(int, int , int);
extern int ui_editevent(int, int , int);
...
if(ui_menu->max>=liemax)
ui_menu->start-=liemax;if(ui_menu->start<0)ui_menu->start=0;
menuref((long)ui_menu);
break;
 ...
 {
 _event(MN_SR,p1,p2);
 DoMenuEvent(MN_SR, ui_menu->gb);
}
 else if(p1==_SLEFT || p1==_SELECT)
 {
 _event(MN_SL,p1,p2);
 DoMenuEvent(MN_SL, ui_menu->gb);
 }
 }
...
dlg->rect.x=0;dlg->rect.y=0;dlg->rect.w=SCRW;dlg->rect.h=SCRH-60;
dlg->color.r=50;dlg->color.g=50;dlg->color.b=50;
...
ui_file->start-=ui_file->liemax;if(ui_file->start<0)ui_file->start=0;
fileref((long)ui_file);
break;
