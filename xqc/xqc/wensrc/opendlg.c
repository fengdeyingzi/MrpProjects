...
extern char WorkFile[128];

static void HideOpenDlg(HWND hWnd)
{
    MakeDialogPath();
	if(!EditLoadFile(SMP_DIALOG_PATH,ED_FILE_RDWR));
		strcpy(WORDOBG->WorkFile,SMP_DIALOG_PATH);
...
static void AddFileToList(int index, PSMPROWDATA pRowData)//2.给列表添加数据的回调函数
{
	...
	pRowData->cols[0].width = (uint16)(SCREEN_WIDTH - 4 - w - 20);  //列表宽度
    ...
    case STR_RENAME://重命名
        {
            ...
            uniText.pText=CharToUnicode(SMP_DIALOG_TEMP_PATH);
			uniText.len=wstrlen((PCWSTR)uniText.pText);
