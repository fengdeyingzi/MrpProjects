...
ST_DATA Sym *sym_free_first = NULL;
...
static inline Sym *sym_malloc(void)
{
    ...
    if (!sym)
        sym = __sym_malloc();
    sym_free_first = sym->nj.next;
    return sym;
}

ST_INLN void sym_free(Sym *sym)
{
    sym->nj.next = sym_free_first;
...
ST_FUNC void lexpand_nr(void)
{
    ...
    if (v == VT_CONST) {
        // eleqian添加强制类型转换 2012-7-15
        vtop[-1].c.ui = (unsigned int)(vtop->c.ull);
        vtop->c.ui = (unsigned int)(vtop->c.ull >> 32);
        vtop->r = VT_CONST;
        ...
        if (c) {
            ...
            if (df) {
                if ((sbt & VT_BTYPE) == VT_LLONG) {
                    if (sbt & VT_UNSIGNED)
                        vtop->c.ld = (long double)vtop->c.ull;  // eleqian添加强制类型转换 2012-7-15
                    else
                        vtop->c.ld = (long double)vtop->c.ll;  // eleqian添加强制类型转换 2012-7-15
                } else if(!sf) {
                    if (sbt & VT_UNSIGNED)
                        vtop->c.ld = vtop->c.ui;
                    else
...
{
    if (type->t & VT_VLA) {
        vset(&int_type, VT_LOCAL|VT_LVAL, type->ref->cd.c);
    } else {
...
static int is_compatible_func(CType *type1, CType *type2)
{
    ...
    if (FUNC_CALL(s1->r) != FUNC_CALL(s2->r))
        return 0;
    /* XXX: not complete */
    if (s1->cd.c == FUNC_OLD || s2->cd.c == FUNC_OLD)
        return 1;
    if (s1->cd.c != s2->cd.c)
        return 0;
    while (s1 != NULL) {
        ...
        if (a == TOK_ENUM) {
            ...
            while (tok != '}') {
                ...
                while (1) {
                    ...
                    if (ad.aligned) {
                        if (align < (int)ad.aligned)  // eleqian添加强制类型转换 2012-7-15
                            align = ad.aligned;
                    } else if (ad.packed) {
                        align = 1;
                    ...
                    if (v == 0 && (type1.t & VT_BTYPE) == VT_STRUCT) {
                        ass = type1.ref;
                        while ((ass = ass->nj.next) != NULL) {
                            ss = sym_push(ass->v, &ass->type, 0, offset + ass->cd.c);
                            *ps = ss;
                            ps = &ss->nj.next;
                        }
                    } else if (v) {
                        ...
                        if (tok == TOK_GOTO) {
                            ...
                            if (tok == '*' && gnu_ext) {
                                ...
                                if (s->r & LABEL_FORWARD) 
                                    s->nj.jnext = gjmp(s->nj.jnext);
                                else
                                    gjmp_addr(s->nj.jnext);
                                next();
                            } else {
                                expect("label identifier");
                            }
                            ...
                            int size_only)
{
    ...
    while (tok == '[' || tok == '.') {
        if (tok == '[') {
            ...
            next();
            index = expr_const();
            if (index < 0 || (s->cd.c >= 0 && index >= s->cd.c))
                expect("invalid index");
            if (tok == TOK_DOTS && gnu_ext) {
                next();
                index_last = expr_const();
                if (index_last < 0 || 
                    (s->cd.c >= 0 && index_last >= s->cd.c) ||
                    index_last < index)
                    expect("invalid index");
            } else {
                index_last = index;
    ...
    if (type->t & VT_VLA) {
        ...
        (sec || !first || tok == '{')) {
            ...
            while (tok != '}') {
                ...
                while (f->nj.next) {
                    ///printf("index: %2d %08x -- %2d %08x\n", f->c, f->type.t, f->nj.next->c, f->nj.next->type.t);
                    /* test for same offset */
                    if (f->nj.next->cd.c != f->cd.c)
                        break;
                    /* if yes, test for bitfield shift */
                    if ((f->type.t & VT_BITFIELD) && (f->nj.next->type.t & VT_BITFIELD)) {
                        int bit_pos_1 = (f->type.t >> VT_STRUCT_SHIFT) & 0x3f;
                        int bit_pos_2 = (f->nj.next->type.t >> VT_STRUCT_SHIFT) & 0x3f;
                        //printf("bitfield %d %d\n", bit_pos_1, bit_pos_2);
                                   ...
                                   int scope)
{
    ...
    if ((type->t & VT_BTYPE) == VT_STRUCT) {
        Sym *field;
        field = type->ref;
        while (field && field->nj.next)
            field = field->nj.next;
        if (field->type.t & VT_ARRAY && field->type.ref->cd.c < 0)
    ...
    if (flexible_array)
        size += flexible_array->type.ref->cd.c * pointed_size(&flexible_array->type);
    /* take into account specified alignment if bigger */
    if (ad->aligned) {
        if ((int)ad->aligned > align)  // eleqian添加强制类型转换 2012-7-15
            align = ad->aligned;
    } else if (ad->packed) {
        align = 1;
    }
    if ((r & VT_VALMASK) == VT_LOCAL) {
            ...
            if (sym) {
                ...
                if (sym->type.t & VT_EXTERN) {
                    ...
                    if ((sym->type.t & VT_ARRAY) && 
                        sym->type.ref->cd.c < 0 &&
                        type->ref->cd.c >= 0)
                        sym->type.ref->cd.c = type->ref->cd.c;
                } else {
        ...
        if (sec) {
            ...
            if (sec->sh_type != SHT_NOBITS && 
                data_offset > (int)sec->data_allocated)  // eleqian添加强制类型转换 2012-7-15
    ...
    if (has_init || (type->t & VT_VLA)) {
        ...
        if (flexible_array)
            flexible_array->type.ref->cd.c = -1;
    }
no_alloc: ;
}
...
static void put_func_debug(Sym *sym)
{
    ...
    sprintf(buf, "%s:%c1", funcname, sym->type.t & VT_STATIC ? 'f' : 'F');
    put_stabs_r(buf, N_FUN, 0, file->line_num, 0,
        cur_text_section, sym->cd.c);
...
static void gen_function(Sym *sym)
{
    ...
    /* end of function */
    /* patch symbol size */
    ((ElfW(Sym) *)symtab_section->data)[sym->cd.c].st_size = 
        ind - func_ind;
...
func_error1:
                ...
                if (btype.t & VT_TYPEDEF) {
                    ...
                    if ((btype.t & VT_EXTERN) || ((type.t & VT_BTYPE) == VT_FUNC) ||
                        ...
                        !has_init && l == VT_CONST && type.ref->cd.c < 0)) {
                            ...
                            if (ad.alias_target) {
                                ...
                                if (!alias_target || !alias_target->cd.c)
                                    tcc_error("unsupported forward __alias__ attribute");
                                esym = &((Elf32_Sym *)symtab_section->data)[alias_target->cd.c];
