...
/* 重新执行i386 / x86_64和苑-m32 / - m64交叉编译器: */
...
extern  void errfunc(void *opaque, const char *msg);
...
static int parse_args(TCCState *s, int argc, char **argv)
{
    ...
    was_pthread = 0; /* 如果命令行包含设置-pthread钥匙吗 */
    optind = 0;
    cstr_new(&linker_arg);
	// 设置信息输出
...
int main(int argc, char **argv)
{
    ...
    /* 如果输出文件没有其他选择,我们输出可执行 */
    if (outfile && output_type == TCC_OUTPUT_MEMORY)
        output_type = TCC_OUTPUT_EXE;

    /* 检查- c一致性:只有单一的文件处理。XXX:检查文件类型 */
    ...
    if (output_type == TCC_OUTPUT_PREPROCESS) {
        if (!outfile) {
            ...
            s->outfile = open(outfile, 1);
            if (!s->outfile)
                tcc_error("无法打开文件 '%s'", outfile);
        }
    }
    ...
    for(i = 0; i < nb_files && ret == 0; i++) {
        ...
        if (filename[0] == '-' && filename[1] == 'l') {
            if (tcc_add_library(s, filename + 2) < 0) {
				tcc_error_noabort("无法找到 %s", filename);
                ret = 1;
            }
        } else {
            if (1 == s->verbose)
   ...
   ret = tcc_compile_string(s,mallocfile("test.c"));
   ret = tcc_output_file(s, "code.o");
