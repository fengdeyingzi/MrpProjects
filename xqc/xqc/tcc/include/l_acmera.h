...
img_quality_type:照片质量类型
	 IMG_QUALITY_LOW:较差的图片质量
	 IMG_QUALITY_NORMAL:一般效果的图片质量
	 IMG_QUALITY_HIGH:高品质的图片质量
...
mode:拍照模式，分连拍和单拍两种模式
	 CAPTURE_MODE_MULTIPLE: 多张连拍模式
	 CAPTURE_MODE_SINGLE: 单张拍照模式,拍照完成以后，如果要继续拍照要调用camera_resume
...
输出:无
返回：摄像头状态值，具体状态值如下:
	 STATUS_UNINIT 未初始化状态
	 STATUS_INIT 初始化成功状态
	 STATUS_INIT_FAILED 初始化失败状态 
	 STATUS_START 启动成功状态
	 STATUS_START_FAILED 启动失败状态
	 STATUS_CAPTURE 拍照成功状态
	 STATUS_CAPTURE_FAILED 拍照失败状态
	 STATUS_PAUSE 暂停状态
	 STATUS_RESUME_FAILED 恢复失败状态
	 STATUS_CLOSE 关闭状态
