...
ST_INLN void define_push(int v, int macro_type, int *str, Sym *first_arg)
{
    ...
    s = define_find(v);
    if (s && !macro_is_equal(s->cd.d, str))
    ...
    for(s = *ptop; s != slast; s = s1) {
        ...
        if (s->r == LABEL_DECLARED) {
            ...
            if (s->cd.c) {
                ...
                1 is put. */
                put_extern_sym(s, cur_text_section, s->nj.jnext, 1);
            }
        }
        /* remove label */
...
float_frac_parse:
        ...
        *q = '\0';
        ...
        if (tok == TOK_CINT || tok == TOK_CUINT)
            tokc.ui = (unsigned int)n;  // eleqian添加强制类型转换 2012-7-15
        else
            tokc.ull = n;
    }
    if (ch)
