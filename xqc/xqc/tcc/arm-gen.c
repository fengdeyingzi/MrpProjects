...
void o(uint32_t i)
{
    ...
    ind1 = ind + 4;
    if (!cur_text_section)
        tcc_error("编译错误! This happens f.ex. if the compiler\n"
...
static uint32_t vfpr(int r)
{
    if(r<TREG_F0 || r>TREG_F7)
		tcc_error("编译器错误!注册 %i 没有vfp注册",r);
    return r-5;
}
#else
...
static int is_float_hgen_aggr(CType *type)
{
    if ((type->t & VT_BTYPE) == VT_STRUCT) {
        ...
        if (btype == VT_FLOAT || btype == VT_DOUBLE) {
            for(; ref && btype == (ref->type.t & VT_BTYPE); ref = ref->nj.next, nb_fields++);
...
void gfunc_call(int nb_args)
{
    ...
    args_size = 0;  // eleqian添加

#ifdef TCC_ARM_HARDFLOAT
    memset(vfp_plan, -1, sizeof(vfp_plan));
    memset(plan2, 0, sizeof(plan2));
    variadic = (vtop[-nb_args].type.ref->cd.c == FUNC_ELLIPSIS);
#endif
    r = vtop->r & VT_VALMASK;
...
default:
    ...
    for(i = 0; i < keep; i++) {
        ...
        if (i < keep - 1 && is_float(vtop->type.t) && (plan2[i] <= plan2[i + 1])) {
            // 添加个条件编译，原来只有TCC_ARM_VFP版本
            // eleqian 2012-7-16
#ifdef TCC_ARM_VFP
            o(0xEEF00A40|(vfpr(plan2[i])<<12)|vfpr(plan2[i]));
#else
            o(0xEEF00A40|(fpr(plan2[i])<<12)|fpr(plan2[i]));
#endif
        }
    }
...
void gfunc_prolog(CType *func_type)
{
    ...
    n = nf = 0;
    variadic = (func_type->ref->cd.c == FUNC_ELLIPSIS);
    ...
    for(sym2=sym->nj.next;sym2 && (n<4 || nf<16);sym2=sym2->nj.next) {
    ...
    o(0xE1A00000); /* nop, leave space for stack adjustment in epilogue */
    {
        ...
        struct avail_regs avregs = AVAIL_REGS_INITIALIZER;
#endif
        while ((sym = sym->nj.next, sym)) {  // 添加逗号后的为避免armcc的警告
            CType *type;
            type = &sym->type;
...
ST_FUNC void gen_cvt_itof1(int t)
{
    ...
    tcc_error("未生效 gen_cvt_itof %x!",vtop->type.t);
}
