...
long ExpressionCoerceInteger(struct Value *Val)
{
    switch (Val->Typ->Base)
    {
		...
		case TypeDP:              return (long)Val->Val->DP;
#endif
        default:                  return 0;
    }
}
...
unsigned long ExpressionCoerceUnsignedInteger(struct Value *Val)
{
    switch (Val->Typ->Base)
    {
		...
		case TypeDP:              return (unsigned long)Val->Val->DP;
#endif
        default:                  return 0;
    }
}
...
#ifndef NO_FP
/* 赋浮点值 */
float ExpressionAssignFP(struct ParseState *Parser, struct Value *DestValue, float FromFP)
{
    if (!DestValue->IsLValue) 
        ProgramFail(Parser, "无法赋值"); 
...
/* 赋浮点值 */
double ExpressionAssignDP(struct ParseState *Parser, struct Value *DestValue, double FromDP)
{
	if (!DestValue->IsLValue) 
		ProgramFail(Parser, "无法赋值"); 

	DestValue->Val->DP = FromDP;
	return FromDP;
}
#endif
...
#ifndef NO_FP
void ExpressionPushFP(struct ParseState *Parser, struct ExpressionStack **StackTop, float FPValue)
{
    struct Value *ValueLoc = VariableAllocValueFromType(Parser, &FPType, FALSE, NULL, FALSE);
    ValueLoc->Val->FP = FPValue;
    ExpressionStackPushValueNode(Parser, StackTop, ValueLoc);
}
void ExpressionPushDP(struct ParseState *Parser, struct ExpressionStack **StackTop, double DPValue)
{
	struct Value *ValueLoc = VariableAllocValueFromType(Parser, &DPType, FALSE, NULL, FALSE);
	ValueLoc->Val->DP = DPValue;
	ExpressionStackPushValueNode(Parser, StackTop, ValueLoc);
}
#endif
...
void ExpressionAssign(struct ParseState *Parser, struct Value *DestValue, struct Value *SourceValue, int Force, const char *FuncName, int ParamNo, int AllowPointerCoercion)
{
    ...
    if (IS_NUMERIC_COERCIBLE(DestValue) && !IS_NUMERIC_COERCIBLE_PLUS_POINTERS(SourceValue, AllowPointerCoercion))
        AssignFail(Parser, "%t 来自 %t 任何类型赋值", DestValue->Typ, SourceValue->Typ, 0, 0, FuncName, ParamNo); 
    ...
    switch (DestValue->Typ->Base)
    {
		...
		case TypeDP:
			...
			DestValue->Val->DP = ExpressionCoerceFP(SourceValue);
			break;
#endif
        case TypePointer:
        ...
        default:
            ...
            if (TopValue->Typ == &FPType)
            {
				...
				if (TopValue->Typ == &DPType)
				{
					...
					switch (Op)
					{
					case TokenPlus:         ResultDP = TopValue->Val->DP; break;
					case TokenMinus:        ResultDP = -TopValue->Val->DP; break;
					default:                ProgramFail(Parser, "无效操作"); break;
					}

					ExpressionPushDP(Parser, StackTop, ResultDP);
				}
            else 
#endif
            if (IS_NUMERIC_COERCIBLE(TopValue))
...
void ExpressionInfixOperator(struct ParseState *Parser, struct ExpressionStack **StackTop, enum LexToken Op, struct Value *BottomValue, struct Value *TopValue)
{
              ...
              (IS_NUMERIC_COERCIBLE(TopValue) && BottomValue->Typ == &FPType) )
    {
        ...
        float ResultFP = 0.0;
        float TopFP = (TopValue->Typ == &FPType) ? TopValue->Val->FP : (float)ExpressionCoerceInteger(TopValue);
        float BottomFP = (BottomValue->Typ == &FPType) ? BottomValue->Val->FP : (float)ExpressionCoerceInteger(BottomValue);
	...
	else if ( (TopValue->Typ == &DPType && BottomValue->Typ == &DPType) ||
		(TopValue->Typ == &DPType && IS_NUMERIC_COERCIBLE(BottomValue)) ||
		(IS_NUMERIC_COERCIBLE(TopValue) && BottomValue->Typ == &DPType) )
	{
		/* 浮点中缀运算 */
		int ResultIsInt = FALSE;
		double ResultDP = 0.0;
		double TopDP = (TopValue->Typ == &DPType) ? TopValue->Val->DP : (double)ExpressionCoerceInteger(TopValue);
		double BottomDP = (BottomValue->Typ == &DPType) ? BottomValue->Val->DP : (double)ExpressionCoerceInteger(BottomValue);

		switch (Op)
		{
		case TokenAssign:               ResultDP = ExpressionAssignDP(Parser, BottomValue, TopDP); break;
		case TokenAddAssign:            ResultDP = ExpressionAssignDP(Parser, BottomValue, BottomDP + TopDP); break;
		case TokenSubtractAssign:       ResultDP = ExpressionAssignDP(Parser, BottomValue, BottomDP - TopDP); break;
		case TokenMultiplyAssign:       ResultDP = ExpressionAssignDP(Parser, BottomValue, BottomDP * TopDP); break;
		case TokenDivideAssign:         ResultDP = ExpressionAssignDP(Parser, BottomValue, BottomDP / TopDP); break;
		case TokenEqual:                ResultInt = BottomDP == TopDP; ResultIsInt = TRUE; break;
		case TokenNotEqual:             ResultInt = BottomDP != TopDP; ResultIsInt = TRUE; break;
		case TokenLessThan:             ResultInt = BottomDP < TopDP; ResultIsInt = TRUE; break;
		case TokenGreaterThan:          ResultInt = BottomDP > TopDP; ResultIsInt = TRUE; break;
		case TokenLessEqual:            ResultInt = BottomDP <= TopDP; ResultIsInt = TRUE; break;
		case TokenGreaterEqual:         ResultInt = BottomDP >= TopDP; ResultIsInt = TRUE; break;
		case TokenPlus:                 ResultDP = BottomDP + TopDP; break;
		case TokenMinus:                ResultDP = BottomDP - TopDP; break;
		case TokenAsterisk:             ResultDP = BottomDP * TopDP; break;
		case TokenSlash:                ResultDP = BottomDP / TopDP; break;
		default:                        ProgramFail(Parser, "无效操作"); break;
		}
...
void ExpressionGetStructElement(struct ParseState *Parser, struct ExpressionStack **StackTop, enum LexToken Token)
{
    ...
    { 
        ...
        if (StructType->Base != TypeStruct && StructType->Base != TypeUnion)
            ProgramFail(Parser, "不能将 '%s' on something that's not a struct or union %s : it's a %t", (Token == TokenDot) ? "." : "->", (Token == TokenArrow) ? "pointer" : "", ParamVal->Typ);
...
void ExpressionParseMacroCall(struct ParseState *Parser, struct ExpressionStack **StackTop, const char *MacroName, struct MacroDef *MDef)
{
    ...
    { 
		/* 这个宏创建一个堆栈帧 */
#ifndef NO_FP
        ExpressionStackPushValueByType(Parser, StackTop, &FPType);  /* 最大的返回类型 */
#else
        ExpressionStackPushValueByType(Parser, StackTop, &IntType);  /* 最大的返回类型 */
#endif
