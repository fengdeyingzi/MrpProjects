...
auto\0break\0case\0char\0continue\0default\0delete\0do\0double\0else\0enum\0extern\0float\0for\0goto\0if\0int\0long\0\
...
TokenAutoType, TokenBreak, TokenCase, TokenCharType, TokenContinue, TokenDefault, TokenDelete, TokenDo,TokenDoubleType, TokenElse, TokenEnumType, TokenExternType,TokenFloatType,
...
static struct ReservedWord ReservedWords[39];
...
enum LexToken LexGetNumber(struct LexState *Lexer, struct Value *Value)
{
    ...
    Value->Typ = &FPType;
    LEXER_INC(Lexer);
    for (FPDiv = 1.0/Base, FPResult = (float)Result; Lexer->Pos != Lexer->End && IS_BASE_DIGIT(*Lexer->Pos, Base); LEXER_INC(Lexer), FPDiv /= (float)Base)
        FPResult += GET_BASE_DIGIT(*Lexer->Pos) * FPDiv;
    
    if (Lexer->Pos != Lexer->End && (*Lexer->Pos == 'e' || *Lexer->Pos == 'E'))
    {
        float ExponentMultiplier = 1.0;
    ...
    { 
        ...
        { 
            switch (Token)
            {
				...
				case TokenDoubleType:           LexValue.Typ = &DPType; break;
#endif
                default: break;
            }
