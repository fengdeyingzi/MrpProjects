...
#endif
#ifdef SIM_MOD
#define  includefile 1  //打开文件库文件模式
#endif

#include "platform.h"
//#define NO_INTERACTIVE
...
#define IS_DP(v) ((v)->Typ->Base == TypeDP)

#define FP_VAL(v) ((v)->Val->FP)
#define DP_VAL(v) ((v)->Val->DP)
#else
#define IS_FP(v) 0
#define FP_VAL(v) 0
#endif
...
union AnyValue
{
    unsigned char Character;              //无符号8bit整型     uint8
    short ShortInteger;                   //有符号16bit整型    int16
    int Integer;                          //有符号32bit整型    int
    long LongInteger;                     //long               int32
    unsigned short UnsignedShortInteger;  //无符号16bit整型    uint16
    unsigned int UnsignedInteger;         //有符号32bit整型    int32
    unsigned long UnsignedLongInteger;    //无符号32bit整型    uint32;
    char *Identifier;
...
extern struct ValueType DPType;
#endif
...
#endif /* INTERPRETER_H */
static struct ValueType * ReturnType;
