...
#define USE_MALLOC_STACK 0                         /* 堆盏内存malloc分配开启 */
...
#else
# ifdef SURVEYOR_HOST
static unsigned char *HeapMemory = (unsigned char *)C_HEAPSTART;      /* 所有的内存  和堆 */
static void *HeapBottom = (void *)C_HEAPSTART + HEAP_SIZE;  /* 堆底(向下生长) */
static void *StackFrame = (void *)C_HEAPSTART;              /* 当前栈帧 */
void *HeapStackTop = (void *)C_HEAPSTART;                   /* 栈顶 */
