...
LRESULT SMP_ProgBar_WndProc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		GAL_Rectangle(PHYSICALGC, x, y, _WIDTH(hWnd), _HEIGHT(hWnd), COLOR_black);
		GAL_FillBox(PHYSICALGC, x+1, y+1, DIV((pData->cur - pData->min) * (_WIDTH(hWnd) - 2), pData->max - pData->min) , _HEIGHT(hWnd) - 2, COLOR_blue);
		break;
	}
