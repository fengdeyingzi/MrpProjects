...
#define STR_DATE_FORMAT	"%d-%d-%d" //display format
#define STR_DATE_LENGTH	10 //max chars count
#define SMP_CALENDAR_HEIGHT	190 //the calendar control height
#define SMP_DATEPICKER_CALENDAR_HEIGHT	(SMP_CALENDAR_HEIGHT + SMP_TOOLBAR_HEIGHT) // the msgbox height

#define SMP_DATEPICKER_MSGBOX		1 //the msgbox id
#define SMP_DATEPICKER_TOOLBAR	2 //the toolbar id
#define SMP_DATEPICKER_CALENDAR 	3 //the calendar id 
...
LRESULT SMP_DatePicker_Calendar_WndProc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
	{
		...
		{
			SGL_ExitModal(hWnd, (WORD)ID_CANCEL,0);
			return 1;
		}else if((wParam == MR_KEY_SELECT || wParam == MR_KEY_SOFTLEFT)){
			SGL_ExitModal(hWnd, (WORD)ID_OK,0);			
			return 1;
		}
		break;
	}
...
LRESULT SMP_DatePicker_WndProc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		fontwh(date, FALSE, (uint16)font, &wt, &ht);
		...
		gbsez2((PSTR)date, (int16)(r.x) , (int16)(r.y + DIV(r.h - ht, 2)), r, color, 0, (uint16)font);
