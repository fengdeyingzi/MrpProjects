...
//the tab bar left/right arrow width as to the bmp icon width
#define SMP_TABWND_ARRBAR_WIDTH		16

//data type for one tab
typedef struct SMP_Tab {
	DWORD bmp; // the icon id
	PCWSTR title; // the tab title
	HWND hWnd; // the child tab window handle
	int32 width; // the width of bmp and title in pixels
}TAB, *PTAB;

//data type for tab window
typedef struct SMP_TabWndData
{
	Sint32  hilight;  // the hilight tab index
	Sint32 count; //current tab count
	TAB tabs[SMP_TABWND_TABCOUNT]; //the tabs window
	int32 titlewidth[SMP_TABWND_TABCOUNT]; //title width
	int32 height; //the tab title content height
	Sint32 first; //the first tab can be shown
	BOOL hasArrBar; //has arrow bar
...
VOID SMP_TabWindow_AddTab(HWND hWnd, DWORD bmp, PCWSTR title, HWND hTab)
{
	...
	{
		int32 w, h;
		fontwh((PSTR)title, TRUE, (uint16)SGL_GetSystemFont(), &w, &h);
		pData->tabs[pData->count].width += w;
...
VOID SMP_TabWindow_SetActiveTabByIndex(HWND hWnd, Sint32 index)
{
	...
	//set the new tab location and show it
	...
	//maybe should adjust the first tab
...
LRESULT SMP_TabWindow_WndProc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		for(i = pData->first; i < pData->count && dx < _WIDTH(hWnd) - SMP_TABWND_ARRBAR_WIDTH; dx += pData->tabs[i++].width)
		{
			...
			if(pData->tabs[i].title && wtab > 0)
			{
				...
				gbsez2((PSTR)pData->tabs[i].title, r.x, r.y, r, color, 1, (uint16)font);
			}
		}
	...
	case WM_MOUSEDOWN:
	{
		...
		//check if click on the arrow bar
		if(pData->hasArrBar
