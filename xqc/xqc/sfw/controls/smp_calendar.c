...
#define SMP_CALENDAR_SPIN_YEAR			1 //year spin wid
#define SMP_CALENDAR_SPIN_YEAR_X			SMP_ITEM_SPACE //year spin left
...
#define SMP_CALENDAR_SPIN_MONTH			2 //month spin wid
...
#define SMP_CALENDAR_WEEKDAY_TITLE_X	SMP_ITEM_SPACE //weekday bar left space
...
typedef struct SMP_CalendarData
{
	...
	int day; //current day
	int32 dayw, dayh; //temp information for the control
}CALDATA, *PCALDATA;
...
LRESULT SMP_Calendar_WndProc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SHOW:
	{
		...
		SMP_Calendar_UpdateValuesToSpins(hWnd);
		fontwh((PSTR)sStrDays[31], FALSE, (uint16)SGL_GetSystemFont(), &pData->dayw, &pData->dayh);
		pData->dayh += 2;
		break;
	}
	...
	case WM_PAINT:
	{
		...
		//draw the title bar
		fontwh((PSTR)SGL_LoadString(STR_YEAR), TRUE, (uint16)font, &wt, &ht);
		...
		gbsez((PSTR)SGL_LoadString(STR_YEAR), (int16)SMP_CALENDAR_STR_YEAR_X, (int16)(y + DIV(SMP_ITEM_HEIGHT-ht, 2)), 0, 0, 0, TRUE, (uint16)font);
		gbsez((PSTR)SGL_LoadString(STR_MONTH), (int16)SMP_CALENDAR_STR_MONTH_X, (int16)(y + DIV(SMP_ITEM_HEIGHT-ht, 2)), 0, 0, 0, TRUE, (uint16)font);

		//draw the weekday title bar
		step = (_WIDTH(hWnd) - 2*SMP_CALENDAR_WEEKDAY_TITLE_X) /7;
		y += SMP_ITEM_HEIGHT;
		for(i = 0; i < 7; i++)
			gbsez((PSTR)SGL_LoadString(STR_SUNDAY + i), (int16)(x+SMP_CALENDAR_WEEKDAY_TITLE_X + i*step), (int16)(y + DIV(SMP_ITEM_HEIGHT - ht, 2)), 0, 0, 0, TRUE, (uint16)font);
		...
		{
			j = MOD(weekday + i - 1, 7);
			gbsez((PSTR)sStrDays[i], (int16)(x + j*step), (int16)y, 0, 0, 0, FALSE, (uint16)font);
			if(i == pData->day)
	...
	case WM_COMMAND:
	{
		...
		//WORD code = HIWORD(wParam); //just one kind notify event
		RECT clip;
