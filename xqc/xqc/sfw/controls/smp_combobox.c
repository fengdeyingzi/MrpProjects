...
{
	DWORD data; // item user data
	PCWSTR str; // item string
}COMBOITEM, *PCOMBOITEM;

//combo data
typedef struct SMP_ComboData
{
	PCOMBOITEM items; //items array
	Sint32 size; // items array size
	Sint32 hili; // high light index in items array
...
static VOID SMP_ComboBox_ShowContentMenu(HWND hWnd)
{
	...
	/*
	 * bad idea for get the menu height, here you must get the men item height and their space and border height.
	 * But This should redesign the menu, so ...
	 */
	fontwh("\x78\x6E\x5B\x9A\x00\x00", TRUE, (uint16)SGL_GetSystemFont(), &w, &h);
	h = (h + 2) * pData->size + 2; // item space is 2, border is 1
...
LRESULT SMP_ComboBox_WndProc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		if(pData->hili >= 0 && pData->hili < pData->size)
		{
			...
			rect.h = _HEIGHT(hWnd);
			fontwh((PSTR)pData->items[pData->hili].str, TRUE, (Uint16)font, &width, &height);

			color.r = PIXEL888RED(_FGCOLOR(hWnd));
			color.g = PIXEL888GREEN(_FGCOLOR(hWnd));
			color.b = PIXEL888BLUE(_FGCOLOR(hWnd));
			gbsez2((PSTR)pData->items[pData->hili].str, rect.x, (int16)(rect.y + DIV(_HEIGHT(hWnd)-height, 2)), rect, color, TRUE, (uint16)font);
		}
