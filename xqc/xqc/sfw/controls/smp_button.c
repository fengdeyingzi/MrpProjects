...
{
	PCWSTR title; // button title string
	DWORD bmp; // button background bitmap in normal state
	DWORD bmp_p; // button background bitmap for pressed state
}BTNDATA, *PBTNDATA;
	...
	case WM_PAINT:
	{
		...
		if(SMP_Button_GetTitle(hBtn))
		{
			...
			if(_IS_SET_ANY(hBtn, WS_FOCUSED))
				GAL_Rectangle(PHYSICALGC, x, y, w, _HEIGHT(hBtn), COLOR_focus);
			
			fontwh((PSTR)SMP_Button_GetTitle(hBtn), TRUE, (Uint16)font, &width, &height);
			...
			if(width > w - 2*SMP_ITEM_CONTENT_MARGIN)
			{
				...
				gbsez2((PSTR)SMP_Button_GetTitle(hBtn), (int16)rect.x, (int16)y, rect, color, 1, (Uint16)font);				
			}else{
				...
				gbsez((PSTR)SMP_Button_GetTitle(hBtn), (int16)x, (int16)y, 
