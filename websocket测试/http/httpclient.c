...
int32 http_ws_run(int32 ip) {
	...
	if (websocket->socket_state == SOCKET_STATE_GETHOST) {
		...
		if(ret == MR_FAILED){
			websocket->socket_state = SOCKET_STATE_ERROR;
			websocket->error = HTTP_ERROR_SEND;
		}else if(ret>0){
		...
		// mrc_printf("--while \r\n");
			...
			if(ret == MR_FAILED){
				websocket->socket_state = SOCKET_STATE_ERROR;
				websocket->error = HTTP_ERROR_RECV;
			}
			temp_int = ret;
	 ...
	 if(websocket->socket_state != SOCKET_STATE_ERROR){
		mrc_timerStart(websocket->timer, 200, ip, http_ws_timer, FALSE); 
		 ...
		 send_result(RESULT_WS_ONERROR, (int32)websocket, "socket连接失败",websocket->error);
	 }
...
void http_ws_exit(int32 ws) {
	...
	if(websocket->timer){
		mrc_timerStop(websocket->timer);
		mrc_timerDelete(websocket->timer);
		websocket->timer = 0;
	}
