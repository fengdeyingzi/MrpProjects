功能机冒泡平台编写说明：
1. 功能机平台使用mrpbuilder编译器进行编译（实际上是调用的ads1.2中的armcc编译器），**采用C98标准，变量声明在函数体最前面**，不支持C99
...
5. mrc_extRecvAppEvent mrc_extRecvAppEventEx 是插件调用函数，保留它，否则编译不过
6. **变量不能在mrc_init之外初始化，请将全局变量初始化赋值写到mrc_init函数里面**
7. 不支持C语言的系统头文件，例如string.h stdlib.h，memcpy memset等函数仍然可以使用，只是被替换成了以下函数：
8. int16 SCRW,SCRH为不可变字段，为屏幕宽度和高度信息，可通过获取它来适配屏幕
9. **gl_drawRect等绘制函数使用 ARGB** 颜色格式，并非565格式
