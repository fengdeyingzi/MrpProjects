...
extern char *mrc_strrchr(const char *s, int c);
extern int32 mrc_wstrlen(const char * txt);
...
#define mrc_sprintf(s, format, ...) (mr_table && mr_table->sprintf ? mr_table->sprintf(s, format, ##__VA_ARGS__) : 0)
#define mrc_printf(format, ...) do { if (mr_table && mr_table->mr_printf) mr_table->mr_printf(format, ##__VA_ARGS__); } while(0)
...
extern outFuncs_st *_start(inFuncs_st *in);
/*
应用初始化函数
