...
typedef enum
{
	NET_EORCODE_OK,
	//GPRS 拨号
	NET_EORCODE_DIALFAILED,					//拨号直接失败，错误会影响应用联网
	//SDK联网鉴权
	NET_EORCODE_AUTHSOCKET_CREATEFAILED,	//SDK联鉴权，请求服务端的Socket创建失败,仅做错误标记，不影响应用联网；
	NET_EORCODE_AUTHSOCKET_DNSFAILED,		//mrc_Socket_ConnectEx时进行SDK联网鉴权，请求服务端的域名解析失败,仅做错误标记，不影响应用联网；
	NET_EORCODE_AUTHSOCKET_CONNFAILED,		//mrc_Socket_ConnectEx时进行SDK联网鉴权，请求服务端Socket连接失败,仅做错误标记，不影响应用联网；
	NET_EORCODE_AUTHIP_NULLIPSTRING,		//mrc_Socket_ConnectEx时进行SDK联网鉴权，输出IP串为空，错误会影响应用联网
	NET_EORCODE_AUTH_SEVERREJECT,			//mrc_Socket_ConnectEx时进行SDK联网鉴权，服务端认定应用非法，错误会影响应用联网
	NET_EORCODE_AUTH_IPNOREG,				//mrc_Socket_ConnectEx时进行SDK联网鉴权，服务端没有登记该IP，错误会影响应用联网
	//SOCKET组
	NET_EORCODE_SOCKET_QUEUEFULL,			//mrc_Socket_Create创建Socket,SDK的Socket队列已满，无法分配出Socket句柄，错误会影响应用联网
	NET_EORCODE_SOCKET_UDP,					//mrc_Socket_Create创建Socket,该socket使用UDP协议，无法支持，错误会影响应用联网
	NET_EORCODE_SOCKET_CREATEFAILED,		//mrc_Socket_Create创建Socket,SDK底层Socket创建失败，可能是Socket资源耗尽，建议关闭所有Socket再重新尝试mrc_Socket_Create，错误影响应用联网；
	NET_EORCODE_SOCKET_SELTIMEREOR,			//mrc_Socket_Create创建Socket,SDK Socket队列的轮询Timer创建失败，应用需要可用使用了过多的Timer句柄资源，该错误码发生后会导致应用退出，一般发生在mrc_Socket_Create调用时，错误会影响应用联网
	NET_EORCODE_SOCKET_CONSOCKETNULL,		//mrc_Socket_ConnectEx时传入的Socket句柄为NULL，错误会影响应用联网
	NET_EORCODE_SOCKET_CONDNSFAILED,		//mrc_Socket_ConnectEx时传入域名地址在进行域名解析时失败，错误会影响应用联网?
	NET_EORCODE_SOCKET_CONFAILED,			//mrc_Socket_ConnectEx时IP CONNECT FAILED
	NET_EORCODE_SOCKET_SENDBUFNULL,		//mrc_Socket_Send时传入的Socket句柄 非法或发送的缓存非法或非TCP连接，错误会影响应用联网
	NET_EORCODE_SOCKET_SENDBUFEXCEPTION,	//mrc_Socket_Send时手机终端底层异常，无法完成发送请求，Scoket句柄将被关闭，错误会影响应用联网
	NET_EORCODE_SOCKET_SENDBUFFULL,			//mrc_Socket_Send时SDK Socket队列的缓存已满，本次发送请求无法完全送出，，错误会影响应用联网
	NET_EORCODE_SOCKET_REVERROR,			//mrc_Socket_Rev时手机终端底层异常 ，本次接收请求无法完完，错误会影响应用联网

	//HTTP组
	NET_EORCODE_HTTP_IPNULL,					//mrc_Http_OpenEx 打开的ipstring为NULL,错误影响应用联网
	NET_EORCODE_HTTP_OBJALLOCATENULL,		//mrc_Http_OpenEx申请的HTTP描述结构内存申请失败，标明目前内存资源十分紧张，错误影响应用联网
	NET_EORCODE_HTTP_SOCKETCREATEFAILED,	//mrc_Http_OpenEx管理的Socket创建失败，错误影响应用联网
	NET_EORCODE_HTTP_POSTSENDERROR,		//mrc_Http_PostEx进行请求发送时Socket队列的缓镼已满，无法进行数据发送，错误影响应用联网	
...
/**
  *\get the last network error code,please see  NET_ERROR_CODE  for ervery error-code definition
  *
  */
int32 mrc_getNetwork_ErrorCode(void);
...
#define SOCKET_SELECT_INTERVAL				100
#endif
...
int mrc_Socket_ConnectNoProxy(PSOCKET socket, char* ip_string,uint16 port, int block);
...
/**
* 仅当mrc_GPRS_Connect拨号失败并且mrc_GPRS_Connect的APN_MODE_E
* 是APN_OVERSEA时，使用mrc_editNetworkAccount函数去设置APN账户。
*/
