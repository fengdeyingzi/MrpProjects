...
static int tpanic (lua_State *L) {
  capp_fprintf(base_stderr, "PANIC: unprotected error in call to Lua API (%s)\n",
                   lua_tostring(L, -1));
...
static char *buildop (Proto *p, int pc, char *buff) {
  ...
  switch (getOpMode(o)) {
    case iABC:
      sprintf(buff+mrc_strlen(buff), "%-12s%4d %4d %4d", name,
              GETARG_A(i), GETARG_B(i), GETARG_C(i));
      break;
    case iABx:
      sprintf(buff+mrc_strlen(buff), "%-12s%4d %4d", name, GETARG_A(i), GETARG_Bx(i));
      break;
    case iAsBx:
      sprintf(buff+mrc_strlen(buff), "%-12s%4d %4d", name, GETARG_A(i), GETARG_sBx(i));
      break;
    case iAx:
      sprintf(buff+mrc_strlen(buff), "%-12s%4d", name, GETARG_Ax(i));
      break;
  }
  return buff;
}
