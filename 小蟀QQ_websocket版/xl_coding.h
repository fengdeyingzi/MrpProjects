...
int UTF8ToUni(char *utf8str, char *unistr, int size)
{
	...
	while (utf8[i] && u < size - 2) {
		if ((utf8[i] & 0x80) == 0) {
			...
			unicode[u + 1] =
			    (unsigned char)((utf8[i] << 6) & 0xff) |
			    (utf8[i + 1] & 0x3f);
			i += 2;
		} else {
			// three bytes 1110... 10... 10...
			unicode[u] =
			    (unsigned char)((utf8[i] << 4) & 0xff) |
			    ((utf8[i + 1] & 0x3f) >> 2);
			unicode[u + 1] =
			    (unsigned char)((utf8[i + 1] << 6) & 0xff) |
			    (utf8[i + 2] & 0x3f);
			i += 3;
		}
...
int UniToUTF8(char *unistr, char *utf8str, int size)
{
	...
	while ((*(unicode + u) || *(unicode + u + 1)) && i < size - 1) {
		if (*(unicode + u) == 0 && *(unicode + u + 1) < 128) {
			...
			// 8 - 11 bits
			utf8[i++] =
			    0xc0 | ((unicode[u] << 2) & 0xff) | (unicode[u + 1]
								 >> 6);
			...
			utf8[i++] =
			    0x80 | ((unicode[u] << 2) & 0x3f) | (unicode[u + 1]
								 >> 6);
			utf8[i++] = 0x80 | (unicode[u + 1] & 0x3f);
		}
...
char *ex_coding(char *text, int len, char *coding, char *tocoding)
{
	...
	if (strcmp(coding, "UTF16-BE") == 0 || strcmp(coding, "utf16-be") == 0
		|| strcmp(coding, "UNICODE") == 0
		|| strcmp(coding, "unicode") == 0) {
			temp_un = text;
			len_un = len;
			relen = len * 2 + 2;
			//printf("待转换编码 unicode");
	} else {
		if (strcmp(coding, "GBK") == 0 || strcmp(coding, "gbk") == 0
			|| strcmp(coding, "GB2312") == 0
			|| strcmp(coding, "gb2312") == 0) {
				//printf("待转换编码 gbk");
				temp_un = gbToUn(text);
				len_un = wstrlen(temp_un);
				relen = len * 2;
		} else if (strcmp(coding, "UTF-8") == 0
			|| strcmp(coding, "utf-8") == 0) {
				//printf("待转换编码 utf");
				len_un = len * 2 + 2;
		...
		} else if (strcmp(coding, "ANSI") == 0
			|| strcmp(coding, "ansi") == 0) {
				//printf("待转换编码 ansi");
				relen = len * 2 + 2;
				temp_un = gbToUn(text);
				len_un = wstrlen(temp_un);
	...
	if (strcmp(tocoding, "unicode") == 0 || strcmp(tocoding, "UNICODE") == 0
		|| strcmp(tocoding, "UTF16-BE") == 0
		|| strcmp(tocoding, "utf16-be") == 0) {
			//printf("转换为unicode");
			return temp_un;
	} else if (strcmp(tocoding, "UTF-8") == 0
		|| strcmp(tocoding, "utf-8") == 0) {
			//printf("转换编码 utf8");
			retext = malloc(relen);
			...
			//retext[relen-4] = '\0';
			return retext;
	} else if (strcmp(tocoding, "gbk") == 0 || strcmp(tocoding, "GBK") == 0
		|| strcmp(tocoding, "gb2312") == 0
		|| strcmp(tocoding, "GB2312") == 0) {
			//printf("转换编码 gbk");
